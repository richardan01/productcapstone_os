...
import {
  ...
  NotionCreateDatabaseItemSchema,
  ...
  NotionCreateDatabaseItemInput,
  NotionGetBlockChildrenInput,
...
import { BlockObjectRequest, CreatePageParameters } from "@notionhq/client/build/src/api-endpoints.js";
...
Returns:
  List of matching pages/databases with titles, IDs, and URLs.`,
    ...
    async (params: NotionSearchInput) => {
      try {
...
Returns:
  Page properties, metadata, and parent information.`,
    ...
    async (params: NotionGetPageInput) => {
      try {
        const page = await getPage(params.page_id) as any;
...
Returns:
  Created page ID and URL.`,
    ...
    async (params: NotionCreatePageInput) => {
      try {
        ...
        if (params.content) {
          createParams.children = markdownToBlocks(params.content) as BlockObjectRequest[];
        }

        const page = await createPage(createParams) as any;
...
Returns:
  Updated page confirmation.`,
    ...
    async (params: NotionUpdatePageInput) => {
      try {
...
Returns:
  List of database items matching the query.`,
    ...
    async (params: NotionQueryDatabaseInput) => {
      try {
...
Returns:
  Created item ID and URL.`,
    ...
    async (params: NotionCreateDatabaseItemInput) => {
      try {
...
Returns:
  List of child blocks with their content.`,
    ...
    async (params: NotionGetBlockChildrenInput) => {
      try {
        const results = await getBlockChildren(
          params.block_id,
          params.start_cursor,
          params.page_size
        );

        const blocks = results.results.map((block: any) => ({
          id: block.id,
          type: block.type,
...
Returns:
  Confirmation with appended block IDs.`,
    ...
    async (params: NotionAppendBlocksInput) => {
      try {
        const results = await appendBlocks(
          params.block_id,
          params.children as BlockObjectRequest[]
        );

        const blockIds = results.results.map((b: any) => b.id);
...
Returns:
  Update confirmation.`,
    ...
    async (params: NotionUpdateBlockInput) => {
      try {
...
Returns:
  Deletion confirmation.`,
    ...
    async (params: NotionDeleteBlockInput) => {
      try {
